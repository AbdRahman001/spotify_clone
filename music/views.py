...
def top_artists():
    ...
    headers = {
	"x-rapidapi-key": "8cdcbe06a4mshfc95b84ac63cbacp11474bjsn9a04af67ab61",
...
def top_tracks():
    ...
    if 'tracks' in data:
        small_data = data['tracks'][:18]

        # id, name, artist, cover url 
        for track in small_data:
            track_id = track['id']
...
def get_track_image(track_id, track_name):
    ...
    if image_links_html:
        image_link = image_links_html['srcset']
    else:
        image_link = '' 

    match = re.search(r'https:\/\/i\.scdn\.co\/image\/[a-zA-Z0-9]+ 640w', image_link)
...
def index(request):
    ...
    first_six_tracks = top_track_list[:10]
    second_six_tracks = top_track_list[6:16]
    third_six_tracks = top_track_list[16:26]
